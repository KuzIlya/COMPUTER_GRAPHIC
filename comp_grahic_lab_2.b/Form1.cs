...
namespace comp_grahic_lab_1
{
    public partial class Form1 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            double W = 1.0, H = 2.0;  // Ширина и высота букв
            double X0 = 1.0, Y0 = 1.0;  // Координаты левого нижнего угла
            double d = 5.0;             // Расстояние между буквами

            DrawLetterK(X0, Y0, W, H);
            DrawLetterI(X0 + W + d, Y0, W, H);
            DrawLetterD(X0 + (W * 2) + (2 * d), Y0, W, H);

            Gl.glFlush();
            AnT.Invalidate();
        }

        void DrawLetterK(double X0, double Y0, double W, double H)
        {

            Gl.glBegin(Gl.GL_LINE_LOOP);
            Gl.glVertex2d(X0, Y0);                          
            Gl.glVertex2d(X0, Y0 + H);                   
            Gl.glVertex2d(X0 + W / 6, Y0 + H);            
            Gl.glVertex2d(X0 + W / 6, Y0 + H * 11 / 15);

            Gl.glVertex2d(X0 + W * 5 / 6, Y0 + H);   
            Gl.glVertex2d(X0 + W, Y0 + H);                 
            Gl.glVertex2d(X0 + W / 6, Y0 + H * 2 / 3);

            Gl.glVertex2d(X0 + W / 6, Y0 + H * 2 / 3);     
            Gl.glVertex2d(X0 + W / 6, Y0 + H * 5 / 9);          
            Gl.glVertex2d(X0 + W, Y0);
            Gl.glVertex2d(X0 + W * 5 / 6, Y0);
            Gl.glVertex2d(X0 + W / 6, Y0 + H * 5 / 12);
            Gl.glVertex2d(X0 + W / 6, Y0);
        ...
        void DrawLetterI(double X0, double Y0, double W, double H)
        {
            Gl.glBegin(Gl.GL_LINE_LOOP);
            Gl.glVertex2d(X0, Y0);
            Gl.glVertex2d(X0, Y0 + H);
            Gl.glVertex2d(X0 + W / 6, Y0 + H);
            Gl.glVertex2d(X0 + W / 6, Y0 + H / 11);

            Gl.glVertex2d(X0 + W * 5 / 6, Y0 + H);
            Gl.glVertex2d(X0 + W, Y0 + H);
            Gl.glVertex2d(X0 + W, Y0);
            Gl.glVertex2d(X0 + W * 5 / 6, Y0);
            Gl.glVertex2d(X0 + W * 5 / 6, Y0 + H * 10 / 12);
            Gl.glVertex2d(X0 + W / 6, Y0);
        ...
        {
            double W = 5.0, H = 5.0;  // Ширина и высота букв
